...
class SingleFieldInstanton(object):
    R"""
    ...
    def initialConditions(self, delta_phi0, rmin, delta_phi_cutoff):
        ...
        def deltaPhiDiff(r_):
            ...
            res = abs(p - self.phi_absMin) - abs(delta_phi_cutoff)
            if np.isnan(res):
                res = np.inf
            return res
    ...
    def integrateAndSaveProfile(self, R, y0, dr,
        ...
        while i < N:
            if iteration > 10000:
                raise Exception("Could not integrate bubble profile." + \
    ...
    def findProfile(self, xguess=None, xtol=1e-4, phitol=1e-4,
        ...
        if max_interior_pts > 0:
            ...
            if R[0] / dx0 <= max_interior_pts:
                n = np.ceil(R[0]/dx0)
                R_int = np.linspace(0, R[0], int(n+1))[:-1]
            else:
                n = max_interior_pts
                # R[0] = dx0 * (n + a*n*(n+1)/2)
                a = (R[0]/dx0 - n) * 2/(n*(n+1))
    ...
    def findAction2(self, profile):
        R"""
        Calculate the Euclidean action for T \neq 0 for the instanton:

        .. math::
          S = 4 \pi \int [ 0.5 * (d\phi/dr)^2 + V(\phi)] r^2 dr 
        ...
        -------
        float
            The Euclidean action for T != 0 in three dimensions.
        """
        r, phi, dphi = profile.R, profile.Phi, profile.dPhi
        area = 4 * np.pi * r**2
        integrand = area * (0.5 * dphi**2 + self.V(phi))
        S = integrate.simps(integrand, r)
        return S
...
class WallWithConstFriction(SingleFieldInstanton):
    ...
    def findAction(self, profile):
        ...
        """
        print("Returning infinite action ?!")
        return np.inf
